/**
 * Action tree, prompts, and helpers (classic script — no export).
 * Loaded before request.js and tooltip.js.
 */
const AIW_ACTIONS = Object.freeze({
...
function aiwIsLeaf(node) {
  return typeof node.template === "string";
}

function aiwFlattenLeaves(nodes, out = []) {
  for (const node of nodes) {
    if (aiwIsLeaf(node)) {
      out.push(node);
    } else if (Array.isArray(node.children)) {
      aiwFlattenLeaves(node.children, out);
    }
  }
  return out;
}

function aiwGetActionTree() {
  return AIW_ACTIONS.tree;
}

function aiwGetDefaultActionId() {
  return AIW_ACTIONS.defaultActionId;
}

function aiwResolveLeaf(actionId) {
  const leaves = aiwFlattenLeaves(AIW_ACTIONS.tree);
  return leaves.find((leaf) => leaf.id === actionId) ?? leaves.find((leaf) => leaf.id === AIW_ACTIONS.defaultActionId) ?? leaves[0];
}

function aiwBuildPrompt(leaf, selection) {
  return leaf.template.replace("{{selection}}", selection);
}

function aiwGetMenuActions() {
  return aiwFlattenLeaves(AIW_ACTIONS.tree).map((leaf) => ({
    id: leaf.id,
    label: leaf.label
  }));
}
